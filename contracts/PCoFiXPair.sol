...
contract PCoFiXPair is CoFiXBase, ICoFiXPair, CoFiXERC20 {
    ...
    address immutable public TOKEN0_ADDRESS; 
    address immutable public TOKEN1_ADDRESS; 
    ...
    // 初始资产比例 - ETH
    uint40 INIT_TOKEN0_AMOUNT;
    
    // 初始资产比例 - TOKEN
    uint40 INIT_TOKEN1_AMOUNT;
    ...
    constructor (
        ...
        uint40 initToken0Amount, 
        uint40 initToken1Amount
    ) {
        name = name_;
        symbol = symbol_;
    ...
    function getInitialAssetRatio() public override view returns (uint initToken0Amount, uint initToken1Amount) {
    ...
    function burn(
        ...
        // 2. 计算净值，根据净值计算等比资金
        // 计算净值
        uint ethBalance = IERC20(TOKEN0_ADDRESS).balanceOf(address(this));
        uint tokenBalance = IERC20(TOKEN1_ADDRESS).balanceOf(address(this));
        uint navps = 1 ether;
        //uint total = totalSupply;
        // if (total > 0) {
        //     // Pt此处没有引入K值，后续需要引入
        ...
        amountTokenOut = amountETHOut * uint(INIT_TOKEN1_AMOUNT) / uint(INIT_TOKEN0_AMOUNT);
        ...
        TransferHelper.safeTransfer(TOKEN1_ADDRESS, to, amountTokenOut);
    ...
    function swap(
        ...
        if (src == TOKEN0_ADDRESS) {
            require(dest == TOKEN1_ADDRESS);
            return swapForToken(amountIn, to, payback);
        } else if (src == TOKEN1_ADDRESS) {
            require(dest == TOKEN0_ADDRESS);
            return swapForETH(amountIn, to, payback);
        }
    ...
    function swapForToken(
        ...
        address to, 
        address payback
    ) public payable override check returns (
        uint amountTokenOut, 
        uint mined
    ) {
        // 1. 调用预言机获取价格
        ...
        //uint tokenBalance1 = IERC20(TOKEN1_ADDRESS).balanceOf(address(this)) - amountTokenOut;
        // 【注意】Pt此处没有引入K值，后续需要引入
        mined = _cofiMint(_calcD(
            address(this).balance, 
            IERC20(TOKEN1_ADDRESS).balanceOf(address(this)) - amountTokenOut, 
            ethAmount, 
            tokenAmount
        ));
    ...
    function swapForETH(
        ...
        //uint tokenBalance1 = IERC20(TOKEN1_ADDRESS).balanceOf(address(this));
        // 【注意】Pt此处没有引入K值，后续需要引入
        mined = _cofiMint(_calcD(
            address(this).balance - amountETHOut, 
            IERC20(TOKEN1_ADDRESS).balanceOf(address(this)), 
            ethAmount, 
            tokenAmount
        ));
    ...
    function _calcD(
        ...
        uint left = ethBalance1 * uint(INIT_TOKEN1_AMOUNT);
        uint right = tokenBalance1 * uint(INIT_TOKEN0_AMOUNT);
        uint numerator;
        if (left > right) {
        ...
        return numerator * ethAmount / (ethAmount * uint(INIT_TOKEN1_AMOUNT) + tokenAmount * uint(INIT_TOKEN0_AMOUNT));
    }
    ...
    function _calcTotalValue(
        ...
        / (
            uint(INIT_TOKEN0_AMOUNT) * tokenAmount 
            + INIT_TOKEN1_AMOUNT * ethAmount
        );
    }
