...
exports.deploy = async function () {
    ...
    let cofixController = await CoFiXController.deploy();
    ...
    await cofixController.initialize(nestPriceFacade.address);
    await usdtPair.init(cofixGovernance.address, 'XT-1', 'XToken-1', usdt.address, BigInt(1000000000), BigInt(2));
    await hbtcPair.init(cofixGovernance.address, 'XT-2', 'XToken-2', hbtc.address, BigInt(20), BigInt(1));
    await nestPair.init(cofixGovernance.address, 'XT-3', 'XToken-3', nest.address, BigInt(1), BigInt(100000));
    await cofiPair.init(cofixGovernance.address, 'XT-4', 'XToken-4', cofi.address, BigInt(1), BigInt(2000));
    await ethAnchor.init(cofixGovernance.address, 1, [eth.address, peth.address], ['1000000000000000000', '1000000000000000000']);
    await usdAnchor.init(cofixGovernance.address, 2, [usdt.address, pusd.address, usdc.address], [1000000, '1000000000000000000', '1000000000000000000']);
